...
(function ($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="assets/js/images/tanda.minus.png"/></div>');
		}
		...
		if ($childNodesCount > 0) {		
			$nodeDiv.next().children('img.cover').click(function () {
				...
				if ($tr.hasClass('contracted')) {
					...
					$(this).attr('src', 'assets/js/images/tanda.minus.png');
					...
					$(this).attr('src', 'assets/js/images/tanda.plus.png');
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', 'assets/js/images/tanda.plus.png');
				} else {
